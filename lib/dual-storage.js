...
class DualStorage extends S3Storage {
    ...
    /**
     * Read from GridFS — mirrors original gridstore-storage createReadStream.
     */
    _createGridFSReadStream(id, attachmentData, options) {
        ...
        if (options && options.startFrom) {
            gridOptions.start = options.startFrom;
        }
        if (options && options.maxLength) {
            gridOptions.end = (options.startFrom || 0) + options.maxLength - 1;
        }

        const stream = this.gridstore.openDownloadStream(id, gridOptions);
